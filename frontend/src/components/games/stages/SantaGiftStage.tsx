...
export const SantaGiftStage: React.FC<SantaGiftStageProps> = ({
  ...
  const handleRingBell = () => {
    ...
    // 4. Santa reacts & prepares gift with sleigh bells chime at t = 950ms
    setTimeout(() => {
      sound.playSleighBells(soundEnabled);
      track('SANTA_APPEAR', 1);

      // 5. Santa drops the glowing gift box down to the snow at t = 1800ms
      setTimeout(() => {
        setGiftThrown(true);
        sound.playMagicChime(soundEnabled);
        track('GIFT_THROWN', 1);

        // 6. Gift lands on ground and opens up, revealing reward at t = 2900ms
        setTimeout(() => {
          setGiftOpened(true);
    ...
    <div className="relative w-full max-w-4xl mx-auto flex flex-col items-center justify-center select-none py-1 sm:py-4 px-2 sm:px-4 touch-manipulation">
      ...
      <div className="text-center mb-2 sm:mb-4 z-10">
        ...
        <p className="text-[11px] sm:text-sm text-cyan-100/80 font-light mt-0.5 drop-shadow">
          Chạm vào chuông vàng thần kỳ — Santa sẽ trao bạn món quà Giáng Sinh may mắn
        </p>
      </div>
      ...
      <div className="relative w-full max-w-2xl rounded-3xl border-2 border-amber-300/40 shadow-[0_20px_60px_rgba(0,0,0,0.85),0_0_30px_rgba(251,191,36,0.2)] overflow-hidden flex flex-col items-center justify-between min-h-[440px] sm:min-h-[500px]">
        ...
        {/* 2. MAJESTIC SANTA & REINDEER SLEIGH IN THE AURORA SKY (ALWAYS VISIBLE & PERFECT ON MOBILE) */}
        <div className="relative w-full z-30 pt-2.5 sm:pt-4 px-2 flex flex-col items-center pointer-events-none">
          {/* Santa Speech Bubble / Status Badge */}
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-1 sm:mb-2 px-3 sm:px-4 py-0.5 sm:py-1 rounded-full bg-gradient-to-r from-red-950/90 via-amber-950/90 to-red-950/90 border border-amber-400/50 shadow-[0_4px_15px_rgba(244,63,94,0.35)] backdrop-blur-md flex items-center gap-1.5"
          >
            <span className="text-sm sm:text-base animate-bounce">🎅</span>
            <span className="text-[10px] sm:text-xs font-sans font-bold text-amber-200 drop-shadow">
              {!bellRung
                ? 'Ho ho ho! Rung chuông vàng để nhận quà nào!'
                : !giftOpened
                ? 'Santa đang mang hộp quà đến cho bạn đây! ✨'
                : 'Ho Ho Ho! Chúc Mừng Giáng Sinh May Mắn! 🎉'}
            </span>
          </motion.div>

          {/* Santa Claus Sleigh with Smooth Flying Physics (Tailored for Mobile & Desktop) */}
          <motion.div
            animate={
              !bellRung
                ? {
                    // Idle floating in the starry aurora sky
                    y: [0, -6, 0],
                    x: [-3, 3, -3],
                    rotate: [-0.6, 0.8, -0.6],
                  }
                : !giftThrown
                ? {
                    // Swooping down gracefully upon bell ring to drop the gift
                    y: [0, -8, 12, 4],
                    x: [0, 6, -4, 0],
                    scale: [1, 1.04, 1.08, 1.03],
                    rotate: [-1, 2, -1, 0],
                  }
                : {
                    // Celebratory hovering above the opened gift
                    y: [0, -4, 0],
                    rotate: [-0.4, 0.4, -0.4],
                    scale: 1,
                  }
            ...
            transition={{
              duration: !bellRung ? 3.0 : !giftThrown ? 1.8 : 2.6,
              repeat: Infinity,
              ease: 'easeInOut',
            }}
            className="relative flex flex-col items-center drop-shadow-[0_12px_28px_rgba(251,191,36,0.9)] max-w-full"
          >
            <img
              src={getAssetUrl('/assets/games/anime_santa_reindeer.png')}
              alt="Ông Già Noel & Tuần Lộc"
              className="w-56 sm:w-72 md:w-84 max-w-[88vw] h-auto object-contain select-none filter brightness-105"
            />

            {/* Glowing stardust exhaust stream behind Santa's sleigh */}
            <div className="absolute -right-4 sm:-right-8 top-1/2 w-28 sm:w-40 h-6 bg-gradient-to-r from-amber-300/80 via-cyan-300/30 to-transparent blur-md rotate-6 animate-stardust-stream pointer-events-none" />

            {/* Sparkle Stars floating around Santa and Reindeer */}
            <div className="absolute -right-2 top-2 text-amber-300 text-xs sm:text-sm animate-ping">✨</div>
            <div className="absolute right-4 -top-2 text-cyan-200 text-xs animate-pulse">★</div>
            <div className="absolute left-2 -top-2 text-amber-300 text-xs animate-bounce">🌟</div>
            <div className="absolute left-12 top-1 text-cyan-300 text-xs animate-pulse">✨</div>
          </motion.div>
        </div>
        ...
        <AnimatePresence>
          {giftThrown && !giftOpened && (
            <motion.div
              ...
              className="absolute top-28 sm:top-36 z-40 pointer-events-none flex flex-col items-center justify-center"
            >
              <motion.div
                animate={{
        ...
        {/* 4. ORNATE WOODEN BEAM & SWINGING ANIME GOLDEN BELL (Interactive Zone) */}
        {!giftOpened && (
          <motion.div
            initial={{ opacity: 1, scale: 1 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            className="relative z-20 flex flex-col items-center my-auto pb-3 sm:pb-5"
          >
        ...
        <div className="w-full flex items-center justify-center p-2 sm:p-3 z-30">
          <AnimatePresence>
            {giftOpened && (
              <motion.div
      ...
      <p className="text-[11px] sm:text-sm text-cyan-200/90 font-sans tracking-wide mt-2 sm:mt-4 text-center drop-shadow">
        {!bellRung
          ? '🔔 Chạm vào chiếc chuông vàng để nhận quà may mắn từ ông già Noel!'
          : giftOpened
          ? '🎉 Chúc mừng bạn đã nhận được món quà may mắn từ ông già Noel!'
          : '✨ Tiếng chuông đang ngân vang... Santa đang trao quà cho bạn!'}
      </p>
    </div>
  );
};
